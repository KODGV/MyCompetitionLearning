...
def analyse(data,name,label='label'):
    result = data[label].groupby(name).agg({'count':'count',
                                              'sum':'sum'})
...
def grp_standard(data,key,names,drop=False):
    for name in names:
        ...
        mean_std = data[name].groupby(key, as_index=False).agg({'mean': 'mean','std': 'std'})
...
def grp_normalize(data,key,names,start=0,drop=False):
    for name in names:
        ...
        max_min = data[name].groupby(key,as_index=False).agg({'max':'max',
                                                              'min':'min'})
